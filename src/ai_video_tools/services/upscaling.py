...
from ai_video_tools.system.processes import CancellationToken, ProcessCancelled, ProcessError, ProcessExecutionError, ProcessResult, ProcessRunner
from ai_video_tools.upscaling.realesrgan import AUTOMATIC_TILE_SIZE, MEMORY_RETRY_TILE_SIZES, UpscalePlan, build_realesrgan_command, build_upscale_plan, is_vulkan_memory_failure
from ai_video_tools.video.frames import FRAME_FILENAME_TEMPLATE, FrameInventoryError, FrameInventoryVerifier
...
class UpscalingExecutor:
    ...
    @staticmethod
    def _emit(callback: ProgressCallback | None, completed: int, total: int, message: str) -> None:
        if callback is not None:
            callback(ProgressEvent(PipelineStage.UPSCALE, completed, total, message))
    ...
    def execute(self, extracted: FrameExtractionResult, job: JobPlan, toolchain: Toolchain, *, workspace: OwnedWorkspace, cancellation: CancellationToken | None = None, progress: ProgressCallback | None = None) -> UpscalingResult:
        ...
        for attempt_number, tile_size in enumerate(tile_sizes, start=1):
            ...
            self._emit(progress, output_count, plan.expected_frame_count, f"Upscaled and verified {output_count} frames")
