...
- Treat blank GUI executable overrides as `PATH` discovery and a blank model-directory override as automatic discovery beside the resolved Real-ESRGAN executable. Validate launches, model files, and Vulkan inference off the GUI thread before atomically saving tool changes. Apply saved changes only to future drafts; queued requests retain their frozen overrides.
- Bind GUI dropped-stream acknowledgement to deterministic keys for the exact reviewed per-clip secondary-stream and chapter inventory. Authoritative queued preflight must reject a changed inventory for another review instead of applying a stale acknowledgement.
...
- Log every FFmpeg, FFprobe, and Real-ESRGAN subprocess launch at INFO as `RUN <shell-quoted argument vector>` immediately before launch. Preserve every exact argument in this local diagnostic record, document that paths may be sensitive, and continue executing only argument arrays with `shell=False`.
